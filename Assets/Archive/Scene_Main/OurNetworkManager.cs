...
public class OurNetworkManager : NetworkManager
{
    ...
    public override void OnServerAddPlayer(NetworkConnection conn, short playerControllerId)
    {
        ...
        if (numConnected % 2 == 0)
        {
            player = (GameObject)Instantiate(m_PlayerPrefab1, new Vector3(0.3f, 0f, 5f), Quaternion.identity);
            numConnected++;
        }
        else if (numConnected % 2 == 1)
        {
            player = (GameObject)Instantiate(m_PlayerPrefab2, new Vector3(0f, 0f, -5f), Quaternion.identity);
            numConnected++;
        }
        else {
